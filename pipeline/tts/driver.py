...
from annotator import annotate, CARTESIA_VOICES
...
def main():
    ...
    out_path = out_dir / f"dialogue_{args.dialogue:02d}_annotated.json"
    out_path.write_text(ann.model_dump_json(indent=2))
    log.info(f"Saved → {out_path}")

    print(f"\n=== Annotated lines ({len(ann.lines)}) ===")
    for i, line in enumerate(ann.lines):
        print(f"\n[{i:>2}] {line.speaker:>14} (voice={line.voice_name})  "
              f"emotion={line.emotion:<18} speed={line.speed}  "
